...
def GOTO(I, X):
    goto = {}
    for heads in list(I.keys()):      # for all left symbols of I
...
def construct_dfa():
    ...
    for i in range(len(C)):
        ...
        for keys in C['I' + str(i)]:
            ...
            for prods in C['I' + str(i)][keys]:
                # print(G)
                zzz = "{:>{width}} ->".format(keys, width=len(max(list(G.keys()), key=len)))
                pd = ""
    ...
    M = [v for v in list(I.values())]
    N = [v for v in list(J.values())]
